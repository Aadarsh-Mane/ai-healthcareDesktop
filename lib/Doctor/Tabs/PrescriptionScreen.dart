...
// Update the _fetchDoctorAdvice method in your DoctorPrescriptionsScreen class
// Add these imports at the top of your file
...
// Update the _fetchDoctorAdvice method in your DoctorPrescriptionsScreen class
  Future<void> _fetchDoctorAdvice(
    ...
    try {
      ...
      if (fileLink != null) {
        // Show options dialog for Open or Print
        _showPdfActionDialog(context, fileLink);
      } else {
...
// Add this new method to show PDF action dialog
  void _showPdfActionDialog(BuildContext context, String fileLink) {
    showDialog(
      context: context,
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Prescription Options'),
            ],
          ),
          content: Text('What would you like to do with the prescription?'),
          actions: [
            TextButton.icon(
              onPressed: () {
            ...
            ElevatedButton.icon(
              onPressed: () async {
                Navigator.of(context).pop();
                await _handlePrintPdf(context, fileLink);
              },
...
// Add this method to handle PDF printing
  Future<void> _handlePrintPdf(BuildContext context, String fileLink) async {
    try {
      ...
      final isAvailable = await Methods.isPrintingAvailable();
      ...
      _showPrintOptionsDialog(context, fileLink);
    } catch (e) {
...
// Add this method to show print options with better error handling
  void _showPrintOptionsDialog(BuildContext context, String fileLink) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ListTile(
                leading: Icon(Icons.preview, color: primaryColor),
                title: Text('Print Preview'),
                subtitle: Text('Preview and print using system dialog'),
                onTap: () async {
                  Navigator.of(context).pop();
                  await _printWithPreview(context, fileLink);
                },
              ),
              Divider(),
              ListTile(
                leading: Icon(Icons.desktop_windows, color: primaryColor),
                title: Text('System Print'),
                subtitle: Text('Use system default printer (Recommended)'),
                onTap: () async {
                  Navigator.of(context).pop();
                  await _printViaSystem(context, fileLink);
                },
              ),
              Divider(),
              ListTile(
                leading: Icon(Icons.print_outlined, color: primaryColor),
                title: Text('Flutter Print'),
                subtitle: Text(
                    'Direct print via Flutter (may not work on all systems)'),
                onTap: () async {
                  Navigator.of(context).pop();
                  await _printDirectly(context, fileLink);
                },
              ),
            ],
          ),
...
// Add these print handling methods with better error handling
  Future<void> _printWithPreview(BuildContext context, String fileLink) async {
    try {
      await Methods().showPrintPreview(fileLink);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _printViaSystem(BuildContext context, String fileLink) async {
    try {
      // Show loading dialog
      showDialog(
        context: context,
      ...
      await Methods().printPdfViaSystem(fileLink);

      Navigator.of(context).pop(); // Close loading dialog

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Document sent to system printer'),
          backgroundColor: success,
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error printing via system: $e'),
          backgroundColor: error,
        ),
      );
    }
  ...
  Future<void> _printDirectly(BuildContext context, String fileLink) async {
    try {
      await Methods().printPdf(fileLink);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
      ...
      // If direct printing fails, suggest alternatives
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Direct printing is not supported on this system.'),
                SizedBox(height: 16),
                Text('Please try one of these alternatives:'),
                SizedBox(height: 8),
                Text('• Use "Print Preview" option'),
                Text('• Use "System Print" option'),
                Text('• Or open in browser and print manually'),
              ],
            ),
            actions: [
              TextButton(
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.of(context).pop();
                  _printWithPreview(context, fileLink);
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: primaryColor,
                  foregroundColor: Colors.white,
                ),
                child: Text('Try Print Preview'),
              ),
            ],
          );
        },
  ...
  Future<void> _printWithPrinterSelection(
      BuildContext context, String fileLink) async {
    try {
      final printers = await Methods.getAvailablePrinters();
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(printer.url ?? 'Default printer'),
                    onTap: () async {
                      Navigator.of(context).pop();
                      try {
                        await Methods()
                            .printToSpecificPrinter(fileLink, printer.name);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Document sent to ${printer.name}'),
                            backgroundColor: success,
                          ),
                        );
                      } catch (e) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text('Error printing to ${printer.name}: $e'),
                            backgroundColor: error,
                          ),
                        );
                      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error getting printers: $e'),
          backgroundColor: error,
        ),
      );
    }
